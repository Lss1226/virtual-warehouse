  ...
  <div class="record">
    ...
    <div class="maintain">
      ...
      <ul v-for="(item,index) in list" :key="index">
        ...
        <li>
          <div>
            <span style="color: coral">{{item.ename}}</span>
          ...
          <div style="margin-top: 0.12rem">
            <span>{{item.maintain_message}}</span>
          </div>
        </li>
      </ul>
    </div>
...
<script>
  ...
  export default {
    ...
    created(){
      document.title='会员维护纪录';
      getDate.selectMemberByMsg(this.id).then((res)=>{//会员维护个人接口
        console.log(res.data.list[0]);
        this.obj=res.data.list[0];
      });
      getDate.selectMemberByRecord(this.id).then((res)=>{//会员维护列表接口
        console.log(typeof res.data.list);
        this.list=res.data.list;
        if (res.data.list===typeof object) {
          setTimeout(()=>{
            this.iNso=true;
          },500);
          setTimeout(()=>{
