...
<script>
  ...
  export default {
    ...
    created(){
      document.title='即将过期合同';
      if (this.id === 1) {
        this.url = 'selectContByDate'
      } else if (this.id === 2) {
        this.url = 'selectContByPayDesc'
      } else if (this.id === 3)  {
        this.url = 'selectContractByCarType1'
      }
      getDate.selectContByDate(this.url,this.pagenum).then(res => {
        console.log(res);
